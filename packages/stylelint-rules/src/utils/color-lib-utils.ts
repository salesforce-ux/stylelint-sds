...
const findClosestColorHook = (color: string, supportedColors, cssProperty: string): { name: string; }[] => {
    ...
    returnStylingHooks.push(...closestHooksWithSameProperty.slice(0, 5).map(h => h.name));
