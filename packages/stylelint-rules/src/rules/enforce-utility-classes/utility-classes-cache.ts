...
export default class CSSClassMatcher {
    ...
    public findExactMatchingClass(requiredSettings: Record<string, string>): string | null {
      // Convert requiredSettings object to the settings format
      const settingsArray = Object.entries(requiredSettings).map(([property, value]) => ({ property, value }));
      ...
      // New helper method to convert rule.nodes to required format
        public findMatchFromNodes(nodes: any[]): string | null {
            ...
            .map((node) => ({ property: node.prop, value: normalizeValue(node.value) }));
