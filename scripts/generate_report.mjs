...
function calculateBatchInfo(totalFiles) {
...
function lintBatch(batch, batchNum) {
  console.log(`lintBatch is called for batch ${batchNum}`);

  return new Promise((resolve, reject) => {
    const outputFile = `${OUTPUT_DIR}/sarif_batch${batchNum}.json`;

    const process = spawn('npx', ['stylelint', ...batch, '--config', CONFIG_FILE, '--formatter', 'json', '--output-file', outputFile]);

    // Consume stdout and stderr to avoid process hanging
    // let stdout = '';
    // let stderr = '';
    ...
    //     console.error(`Batch ${batchNum} timed out.`);
    //     process.kill(); // Kill the hanging process
    ...
    //     clearTimeout(timeout); // Clear the timeout once the process exits
    ...
    //         resolve();
    //     } else {
    //         console.error(`Batch ${batchNum} failed with exit code ${code}.`);
